...
이 예제는 현재의 기능별 REST 보상 지급 엔드포인트를 사용합니다.

!!! tip "ID 조회"
    `ItemID`는 [paldeck.cc/items](https://paldeck.cc/items), `PalID`는 [paldeck.cc/pals](https://paldeck.cc/pals), `TechID`는 [paldeck.cc/technology](https://paldeck.cc/technology)에서 확인하세요.
...
# 실제 토큰을 코드에 저장하지 마세요. 환경 변수나 비밀 정보 관리 도구를 사용하세요.
