...
??? faq "我不小心封禁了自己/其他人。如何解除封禁？"


    账号封禁请使用 `/unban &lt;UserId&gt;`{.var-command}，IP 封禁请使用 `/unbanip &lt;IP&gt;`{.var-command}。PalDefender 的封禁记录保存在 <span class="path-partial">../Pal/Binaries/Win64/PalDefender/</span><span class="file-partial">Banlist.json</span>。如果要手动编辑该文件，请先停止服务器，或在编辑后重新加载配置。




??? faq "<span class='pd-badge pd-badge--deprecated'>已废弃</span> 旧版通过 Config.json 清理 IP 封禁"


    旧版 Wiki 曾提示管理员从 <span class="path-partial">../Pal/Binaries/Win64/PalDefender/</span><span class="file-partial">Config.json</span> 删除 IP 封禁。该位置已不再用于封禁记录。请改用 `/unbanip &lt;IP&gt;`{.var-command}，或编辑 <span class="path-partial">../Pal/Binaries/Win64/PalDefender/</span><span class="file-partial">Banlist.json</span>。




??? faq "我添加或修改了 PalTemplate/PalSummon 文件，但命令失败。"
    ...
    <li>使用不带路径的文件名，例如 `/summon ArenaBoss`{.var-command}。</li>
...
??? faq "命令应该使用哪种 ID：UserId、PlayerUId、名称还是 SteamID？"


    大多数管理员命令需要玩家的 UserId，例如 <code>steam_...</code> 或 <code>gdk_...</code>。在游戏内使用 `/iwantplayerlist`{.var-command} 可在玩家列表中显示 ID；如果已启用，也可以使用 REST/API 工具。




??? faq "为什么在聊天中不需要参数的命令，通过 RCON 执行时需要坐标或 UserId？"


    RCON 没有游戏内玩家角色，因此 PalDefender 无法推断你的位置或目标。对于 `/getpos`{.var-command}、`/tp`{.var-command}、`/spawnpal`{.var-command} 以及基地位置相关命令，请明确提供目标玩家或坐标。




??? faq "REST API 返回 401 或 403，我应该检查什么？"
